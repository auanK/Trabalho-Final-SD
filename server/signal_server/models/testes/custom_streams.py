...
# Assume que UserRecord está em models/UserRecord.py
from models.UserRecord import UserRecord # Ajuste o import se necessário

# Formatos Struct (Big Endian - Network Order)
FMT_COUNT = "!H" # Unsigned Short (2 bytes) para contagens e tamanhos

class UserRecordOutputStream(io.BufferedIOBase):
    """
    Subclasse de OutputStream para escrever um array de UserRecord
    em um formato binário específico[cite: 7].
    Formato: [Num Objetos (H)] -> Para cada objeto:
               [Tam Nick (H)][Nick Bytes]
               [Tam Name (H)][Name Bytes]
               [Tam Desc (H)][Desc Bytes]
    """
    def __init__(self, records_array: List[UserRecord], num_objects_to_send: int, dest_stream: BinaryIO): # [cite: 9, 10, 11]
        """
        ...
        self._serialize_all() # Preenche o buffer
    ...
    def _serialize_all(self):
        ...
        # Escreve o número total de objetos a serem enviados [cite: 10]
    ...
    def write(self, b: bytes = None) -> int:
        ...
        try:
            ...
            if hasattr(self.dest_stream, 'flush'): # Nem todos os streams têm flush (ex: bytesio)
            ...
            if written == len(self._buffer):
                 ...
                 return 0 # Indica que nada foi escrito (pode ser um erro)

        except Exception as e:
            print(f"Erro ao escrever no stream de destino: {e}")
            self._buffer = bytearray() # Limpa buffer em caso de erro
            raise

    # Métodos necessários por BufferedIOBase
    def readable(self) -> bool: return False
    def seekable(self) -> bool: return False
    def writable(self) -> bool: return True


# custom_streams.py (continuação)

class UserRecordInputStream(io.BufferedIOBase):
    """
    Subclasse para InputStream que lê dados gerados pelo
    UserRecordOutputStream[cite: 21].
    """
    def __init__(self, source_stream: BinaryIO): # [cite: 23]
        """
        Construtor[cite: 23].
        :param source_stream: InputStream de origem[cite: 23].
        """
        super().__init__()
        self.source_stream = source_stream
        self.num_objects = -1 # Número total de objetos no stream (a ser lido)
    ...
    def read_next_record(self) -> Optional[UserRecord]:
        ...
        if self.num_objects == -1:
            try:
                ...
                # print(f"[DEBUG] Stream contém {self.num_objects} registros.")
    ...
    def read(self, size: int = -1) -> bytes:
        # Implementação básica apenas para compatibilidade,
        # ler por registro é mais útil aqui.
