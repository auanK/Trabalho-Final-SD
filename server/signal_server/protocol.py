...
class BaseProtocol(ABC):
    """
    Superclasse que define a estrutura de um protocolo binário.
    Contém lógica de serialização de tipos básicos e define a estrutura da mensagem.
    """
    ...
    def serialize_string(self, s: str) -> bytes:
        """Serializa uma string: Tamanho (2 bytes, !H) + String UTF-8"""
    ...
    def deserialize_string(self, buffer: bytes, offset: int) -> Tuple[str, int]:
        """Desserializa uma string: Lê Tamanho (2 bytes) + Lê String UTF-8"""
    ...
    def deserialize_string_list(self, buffer: bytes, offset: int) -> Tuple[List[str], int]:
        """Desserializa uma lista de strings"""
        if len(buffer) < offset + struct.calcsize(self.FMT_COUNT):
    ...
    def create_message(self, command_code: CommandCode, payload_dict: Dict) -> bytes:
        """
        Método principal para criar uma mensagem.
        Ele chama o método 'serialize_payload'
        e então constrói o cabeçalho.
        """
...
class VoipProtocol(BaseProtocol):
    """
    Subclasse que implementa a serialização/desserialização
    específica para o protocolo VoIP.
    """
    
    def serialize_payload(self, command_code: CommandCode, payload: dict) -> bytes:
        """Implementa a serialização de TODOS os payloads do VoIP."""
        b = bytearray()
        try:
    ...
    def deserialize_payload(self, command_code: CommandCode, payload_bytes: bytes) -> dict:
        """Implementa a desserialização de TODOS os payloads do VoIP."""
        payload = {}
        offset = 0
        try:
            # --- Payloads Cliente -> Servidor ---
