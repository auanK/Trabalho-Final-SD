...
# Adiciona um pedido de amizade com status 'pending'.
...
# Atualiza um pedido de amizade (status 'pending') para 'accepted' ou 'rejected'.
...
def get_friends_list_db(nickname):
    ...
    try:
        ...
        friends.extend([row[0] for row in cursor.fetchall()])
        return list(set(friends)) # Remove duplicatas
    except sqlite3.Error as e:
